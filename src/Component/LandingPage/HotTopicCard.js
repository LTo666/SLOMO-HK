...
export default function HotTopicCard({ title, image }) {
  ...
  const imgClassName = "h-full w-full absolute top-0 left-0 object-cover"
  ...
  return (
    <div>
      <div className={`${divClassName} ${style.hoverCard}`}>
        <div className={`absolute w-full h-full transition-all duration-300 ease-in-out ${style.blur} ${style.gradient}`}>
        ...
        <p className={style.hoverText}>閱讀更多</p>
      </div>
    </div>
  )
}
